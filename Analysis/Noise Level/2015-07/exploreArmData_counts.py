...
inamp_gain = np.array([80.0])  #assumed for his inamp
#inamp_gain = np.array([1.0])  #assumed for his inamp
adc_gain = 1
...
bp_cutoff_Hz = np.array([0.1, 50.0])
...
for Ichan in range(nchan):
    ...
    plt.xlabel("Time (sec)")
    txt = "InAmp Gain = " + str(inamp_gain[0]) + '\nADC Gain = ' + str(adc_gain)
    ax.text(0.97, 0.95,
...
plt.tight_layout()
plt.savefig('FIGs\\' + fname[0:-4] + '.png') 
